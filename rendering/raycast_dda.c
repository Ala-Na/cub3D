...
/*   Updated: 2022/01/14 16:51:04 by anadege          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_differential_diagnosis_analysis(t_data *data, t_ray *ray)
{
	...
	{
		ray->step.x = -1;
		ray->side_dist.x = (data->player->pos.x - ray->box.x)
			* ray->delta_dist.x;
	}
	else
	{
		ray->step.x = 1;
		ray->side_dist.x = (ray->box.x + 1.0 - data->player->pos.x)
			* ray->delta_dist.x;
	}
	if (ray->dir.y < 0)
	{
		ray->step.y = -1;
		ray->side_dist.y = (data->player->pos.y - ray->box.y)
			* ray->delta_dist.y;
	}
	else
	{
		ray->step.y = 1;
		ray->side_dist.y = (ray->box.y + 1.0 - data->player->pos.y)
			* ray->delta_dist.y;
	}
...
void	calculate_distance_deltas(t_data *data, t_img *img, t_ray *ray)
{
	...
	if (isnan(ray->delta_dist.x) != 0 || isnan(ray->delta_dist.y) != 0)
	{
		...
		ft_error(ERROR_CALCULATION, data);
	}
}
...
void	raycasting_algorithm(t_data *data, t_img *img)
{
	...
	while (ray.screen_x < data->screen_width)
	{
		...
		ray.dir.x = data->player->dir.x + data->player->cam_plane.x
			* ray.cam_pos_x;
		ray.dir.y = data->player->dir.y + data->player->cam_plane.y
			* ray.cam_pos_x;
		calculate_distance_deltas(data, img, &ray);
		ray.side = init_differential_diagnosis_analysis(data, &ray);
		ray.wall_height = calculate_wall_height(&ray, data->screen_height);
		raycast_textured_wall(data, &ray, img);
		ray.screen_x++;
	}
}
